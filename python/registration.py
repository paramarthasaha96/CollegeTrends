...
def complete(form, type):
	reqFields = ['mobile', 'pwd']
	if type == 0:
		reqFields.append('stream')
		reqFields.append('year')
	elif type == 1:
		reqFields.append('stream')
...
def register_student():
	...
	if request.method == 'POST':
		if not complete(request.form, 0):
			...
			if validate_fields(request.form['mobile'], request.form['email']) == True:
				...
				user = Users(mobile = request.form['mobile'], fname = request.form['fname'],
					lname = request.form['lname'], email = e_mail, pwd = request.form['pwd'], role = Users.ROLE_STUDENT)
				stream_to_int = stream_list[request.form['stream']]
				try:
					db.session.add(user)
					student = Students(user.id, stream_to_int, int(request.form['year']))
...
def register_faculty():
	stream_list = {"CSE": 0, "EE": 1, "ECE": 2, "ME": 3}
	if request.method == 'POST':
		if not complete(request.form, 1):
			return jsonify({ 'response': 1, 'msg': "Incomplete form" })
		else:
			if 'email' not in request.form:
				e_mail = ''
			...
			if validate_fields(request.form['mobile'], request.form['email']) == True:
				db.create_all()
				user = Users(mobile = request.form['mobile'], fname = request.form['fname'],
					lname = request.form['lname'], email = e_mail, pwd = request.form['pwd'], role = Users.ROLE_FACULTY)
				stream_to_int = stream_list[request.form['stream']]
				try:
					db.session.add(user)
					faculty = Faculty(user.id, stream_to_int)
...
def register_admin():
	if request.method == 'POST':
		if not complete(request.form, 2):
			return jsonify({ 'response': 1, 'msg': "Incomplete form" })
		else:
			if 'email' not in request.form:
				e_mail = ''
			...
			if validate_fields(request.form['mobile'], request.form['email']) == True:
				db.create_all()
				user = Users(mobile = request.form['mobile'], fname = request.form['fname'],
					lname = request.form['lname'], email = e_mail, pwd = request.form['pwd'], role = Users.ROLE_ADMIN)
				try:
					db.session.add(user)
					admin = Admin(user.id)
